<!-- src/components/CabinetsComponent.vue -->
<template>
  <div>
    <main>
      <div class="source-container">
        <input
          type="text"
          v-model="newCabinetName"
          placeholder="Добавить Кабинет"
          class="source-add"
          @keyup.enter="addCabinet"
        />
        <div class="table-cells">
          <div class="cell" v-for="cabinet in cabinets" :key="cabinet.id">
            <div class="cell-text">
              <p class="h4">{{ cabinet.name }}</p>
            </div>
            <div class="buttons">
              <div class="buttons-wrapper">
                <img
                  src="/src/assets/Edit.svg"
                  alt="edit"
                  class="button"
                  @click="startEdit(cabinet)"
                />
                <img
                  src="/src/assets/Trash.svg"
                  alt="delete"
                  class="button"
                  @click="deleteCabinet(cabinet.id)"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </main>
  </div>
</template>

<script>
import { useCabinetsStore } from '@/stores/cabinets';

export default {
  name: 'CabinetsComponent',
  data() {
    return {
      newCabinetName: '',
      editingCabinet: null,
    };
  },
  computed: {
    cabinets() {
      return useCabinetsStore().cabinets;
    },
  },
  methods: {
    addCabinet() {
      if (this.newCabinetName.trim()) {
        useCabinetsStore().addCabinet(this.newCabinetName);
        this.newCabinetName = '';
      }
    },
    startEdit(cabinet) {
      this.editingCabinet = cabinet;
      this.newCabinetName = cabinet.name;
    },
    updateCabinet() {
      if (this.editingCabinet && this.newCabinetName.trim()) {
        useCabinetsStore().updateCabinet(this.editingCabinet.id, this.newCabinetName);
        this.editingCabinet = null;
        this.newCabinetName = '';
      }
    },
    deleteCabinet(id) {
      useCabinetsStore().deleteCabinet(id);
    },
  },
  mounted() {
    useCabinetsStore().fetchCabinets();
  },
};
</script>

<style scoped>
@font-face {
  font-family: 'Inter';
  src: url('/fonts/Inter-Regular.woff2') format('woff2'),
       url('/fonts/Inter-Regular.woff') format('woff');
  font-weight: 400;
  font-style: normal;
}

@font-face {
  font-family: 'Inter';
  src: url('/fonts/Inter-Bold.woff2') format('woff2'),
       url('/fonts/Inter-Bold.woff') format('woff');
  font-weight: 700;
  font-style: normal;
}

body {
  font-family: 'Inter', sans-serif;
  margin: 0;
}

body, p, .h1, .h2, .h3, .h4 {
  font-family: 'Inter', sans-serif;
}

/* Стили для заголовков */
.h1 {
  font-size: 32px;
  margin: 2px 0 2px 0;
}

.h2 {
  font-size: 24px;
  margin: 2px 0 2px 0;
}

.h3 {
  font-size: 18px;
  margin: 2px 0 2px 0;
}

.h4 {
  font-size: 16px;
  margin: 8px 0 8px 0;
}

.source-add {
  font-size: 16px;
  transition: all 0.1s ease-in-out;
}

input {
  padding: 8px;
  border: 2px solid #1E1E1E;
  border-radius: 8px;
}

.source-add:focus {
  outline: none;
  border-color: #2C2C2C;
  box-shadow: 0 0 0 0.25rem rgba(194, 194, 194, 0.25);
}

.source-container {
  display: flex;
  flex-direction: column;
  width: 100%;
}

.table-cells {
  margin-top: 16px;
  border: 2px solid #D9D9D9;
  border-radius: 16px;
  padding-left: 16px;
  padding-right: 16px;
  padding-bottom: 8px;
}

.cell {
  border-bottom: #D9D9D9 1px dotted;
  color: #1E1E1E;
  display: flex;
  justify-content: space-between;
  align-items: center;
}

.cell p {
  font-weight: 500;
}

.buttons-wrapper {
  display: flex;
  justify-content: space-between;
  align-items: center;
}

.button {
  margin-left: 8px;
  transition: all 0.1s ease-in-out;
  margin-top: 4px;
}

.button:hover {
  transform: translateY(-2px);
}

.button:active {
  transform: translateY(2px);
  box-shadow: none;
}
</style>
